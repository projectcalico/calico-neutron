...
class AgentSchedulerDbMixin(agents_db.AgentDbMixin):
    ...
    def wait_down_agents(self, agent_type, agent_dead_limit):
        ...
        if timeutils.total_seconds(tdelta) > cfg.CONF.agent_down_time:
            LOG.warn(_("Time since last %s agent reschedule check has "
...
class DhcpAgentSchedulerDbMixin(dhcpagentscheduler
    ...
    def start_periodic_dhcp_agent_status_check(self):
        if not cfg.CONF.allow_automatic_dhcp_failover:
            LOG.info(_("Skipping periodic DHCP agent status check because "
    ...
    def _schedule_network(self, context, network_id, dhcp_notifier):
        LOG.info(_("Scheduling unhosted network %s"), network_id)
        try:
            ...
            if not agents:
                LOG.info(_("Failed to schedule network %s, "
            ...
            if not dhcp_notifier:
                return
            for agent in agents:
                LOG.info(_("Adding network %(net)s to agent "
            ...
            LOG.exception(_("Failed to schedule network %s"), network_id)
    ...
    def remove_networks_from_down_agents(self):
        ...
        for binding in self._filter_bindings(context, down_bindings):
            LOG.warn(_("Removing network %(network)s from agent %(agent)s "
            ...
            try:
                ...
                LOG.exception(_("Unexpected exception occured while "
